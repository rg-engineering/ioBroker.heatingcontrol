...
async function SetTrigger() {
    ...
    try {
        ...
        parentAdapter.subscribeStates("vis.RoomValues.TemperatureIfNoHeatingPeriod");
...
async function SetVis() {
    ...
    try {
        ...
        await parentAdapter.setStateAsync(
            "vis.RoomValues.TemperatureIfNoHeatingPeriod",
            { ack: true, val: parentAdapter.config.ThermostatModeIfNoHeatingperiod == 1
                ? await getStateValOrZero(`Rooms.${ChoosenRoom}.TemperatureIfNoHeatingPeriod`)
                : 0 }
        );
        ...
        if (parentAdapter.config.UseVisFromPittini) {
            ...
            if (ids[4] == "ProfileName") {
                ...
                if (ids[4] === "MinimumTemperature") {
                    ...
                    parentAdapter.log.debug("HandleStateChanges TemperatureIfNoHeatingPeriod ");
                    if (RefreshingVis === false) {
                        await SetRoomValue("TemperatureIfNoHeatingPeriod", state.val);
                    }
