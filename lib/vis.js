...
async function CreateWindowStatesTable() { // Erzeugt List mit Räumen und Fensterstatus
    ...
    try {
        ...
        if (parentAdapter.config.VisUseSimple) {
            ...
            HtmlTable += "<table>";
            HtmlTable += "<tr>";
            HtmlTable += "<th> " + GetTranslation("room", SystemLanguage) + " </th > ";
            HtmlTable += "<th> " + GetTranslation("status", SystemLanguage) + " </th>";
            HtmlTable += "<th> " + GetTranslation("since", SystemLanguage) + " </th>";
            HtmlTable += "</tr>";
            ...
            for (let x = 0; x < GetNumberOfActiveRooms(); x++) {
                ...
                if (theRoom.hasWindowSensors) {
                    ...
                    HtmlTable += "<tr>";
                    HtmlTable += "<td>" + theRoom.Name + " </td>";

                    if (WindowState[x] == true) {
                        HtmlTable += "<td> " + GetTranslation("opened", SystemLanguage) + " </td>";
                        OpenWindowRoomCount++;
                    }
                    else {
                        HtmlTable += "<td> " + GetTranslation("closed", SystemLanguage) + " </td>";
                    }
                    HtmlTable += "<td>" + WindowStateTimeStamp[x] + "</td>";
...
async function SetProfileValueList() { //Einträge für Vis Profil Valuelist erstellen
    ...
    try {
        ...
        for (let x = 1; x <= parseInt(parentAdapter.config.NumberOfProfiles, 10); x++) {
            ...
            if (profileName != null && typeof profileName != undefined) {
...
async function SetTempDecreaseModeValueLists() { //Setzt die Vorgabewerte der Valuelists je nach gewähltem DecreaseMode
    ...
    try {
        ...
        switch (TempDecreaseMode) {
            case "relative":
                if (parseInt(parentAdapter.config.VisMinDecRelTemp) > 0 && parseInt(parentAdapter.config.VisMaxDecRelTemp) > 0) {
                    ...
                    let tempValue = parseInt(parentAdapter.config.VisMinDecRelTemp);

                    while (tempValue <= parseInt(parentAdapter.config.VisMaxDecRelTemp)) {
                        RelTempValueListValue += tempValue + ";";
                        RelTempAddValueListText += "+" + tempValue + "°C;";
                        RelTempDivValueListText += "-" + tempValue + "°C;";
                        tempValue += Number(parentAdapter.config.VisStepWidthDecRelTemp);
                    }
                }
            ...
            case "absolute":
                if (parseInt(parentAdapter.config.VisMinDecAbsTemp) > 0 && parseInt(parentAdapter.config.VisMaxDecAbsTemp) > 0) {
                    ...
                    let tempValue = parseInt(parentAdapter.config.VisMinDecAbsTemp);

                    while (tempValue <= parseInt(parentAdapter.config.VisMaxDecAbsTemp)) {
                        AbsTempValueListValue += tempValue + ";";
                        AbsTempValueListText +=  tempValue + "°C;";
                        tempValue += Number(parentAdapter.config.VisStepWidthDecAbsTemp);
                    }
                }
        ...
        if (parseInt(parentAdapter.config.VisMinOverrideTemp) > 0 && parseInt(parentAdapter.config.VisMaxOverrideTemp) > 0) {
            ...
            let tempValue = parseInt(parentAdapter.config.VisMinOverrideTemp);

            while (tempValue <= parseInt(parentAdapter.config.VisMaxOverrideTemp)) {
                OverrideTempValueListValue += tempValue + ";";
                OverrideTempValueListText += tempValue + "°C;";
                tempValue += Number(parentAdapter.config.VisStepWidthOverrideTemp);
            }
        }
        ...
        if (parseInt(parentAdapter.config.VisMinProfilTemp) > 0 && parseInt(parentAdapter.config.VisMaxProfilTemp) > 0) {
            ...
            TempValueListValue = "";
            TempValueListText = "";
            let tempValue = parseInt(parentAdapter.config.VisMinProfilTemp);
            while (tempValue <= parseInt(parentAdapter.config.VisMaxProfilTemp)){
                TempValueListValue += tempValue + ";";
                TempValueListText += tempValue + "°C;";
                tempValue += Number(parentAdapter.config.VisStepWidthProfilTemp);
            }
        }
...
async function CreateCurrentTimePeriodTrigger() {
    ...
    try {
        ...
        if (OldChoosenRoom != "none" && OldChoosenRoom != "") { //Wenn kein Oldroom angegeben kein unsubscribe machen
            ...
            parentAdapter.unsubscribeStates("Rooms." + OldChoosenRoom + ".CurrentTimePeriod"); //Trigger auf vorherigen Raum löschen
        ...
        parentAdapter.subscribeStates("Rooms." + ChoosenRoom + ".CurrentTimePeriod");
    }
    catch (e) {
...
async function SetTrigger() {
    ...
    try {
        ...
        for (let x = 0; x <= Rooms.length - 1; x++) {
            let id = "Rooms." + Rooms[x] + ".WindowIsOpen";
            ...
            id = "Rooms." + Rooms[x] + ".StatusLog";
            parentAdapter.log.debug("subscribe " + id);
            parentAdapter.subscribeStates(id); //Wenn Log sich ändert
        }
...
async function SetWindowState() { //Fenster offenstatus für einzelnen Raum/Fenster festlegen
    ...
    if (ChoosenRoom.length > 0) {
        ...
        if (temp != null && typeof temp != undefined) {
            const id = "vis.RoomValues.WindowIsOpen";
            await parentAdapter.setStateAsync(id, { ack: true, val: temp.val });
        }
        else {
            parentAdapter.log.error("SetWindowState: could not read " + state);
        }
    }
}

//==========================================================================================================================
async function SetStatusLog() { 

    if (ChoosenRoom.length > 0) {
        const state = "Rooms." + ChoosenRoom + "." + "StatusLog";
        //parentAdapter.log.debug("Reaching SetStatusLog for " + state);
        const temp = await parentAdapter.getStateAsync(state);
        ...
        if (temp != null && typeof temp != undefined) {
            ...
            parentAdapter.log.error("SetStatusLog: could not read " + state);
        }
    }
}
...
async function HandleStateChangeVis(id, state) {
    ...
    try {
        if (parentAdapter.config.UseVisFromPittini) {
            ...
            else if (ids[4] === "CurrentTimePeriod") {
                parentAdapter.log.debug("HandleStateChanges CurrentTimePeriod " + state.val);
                if (RefreshingVis === false) {
            ...
            else if (ids[4] == "StatusLog") {
                //parentAdapter.log.debug("HandleStateChanges StatusLog " + state.val);
                //Raum suchen 
                const curRoom = ids[3];
                const Rooms = UsedRooms.split(";");
