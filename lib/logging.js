...
function ActorLog(actor, target) {
    ...
    //todo actor undefined see https://forum.iobroker.net/assets/uploads/files/1707641550020-img_5923.png
...
function sendNotification(msg) {
    try {
        if (parentAdapter.config.notificationEnabled) {
            parentAdapter.log.debug("send notfications, type " + parentAdapter.config.notificationsType);
            if (parentAdapter.config.notificationsType == "Telegram") {

                if (parentAdapter.config.telegramWaitToSend > 0) {
                    if (TelegramTimerId == null) {
                        TelegramMessage  = msg;
                        TelegramTimerId = parentAdapter.setTimeout(sendNotificationTelegram, parentAdapter.config.telegramWaitToSend * 1000);
                    } else {
                        TelegramMessage += "\r\n" + msg;
                    }
                } else {
                ...
                if (parentAdapter.config.emailWaitToSend > 0) {
                    ...
                    if (EmailTimerId == null) {
                        ...
                        EmailMessage = msg;
                        EmailTimerId = parentAdapter.setTimeout(sendNotificationEmail, parentAdapter.config.emailWaitToSend * 1000);
                    } else {
                ...
                if (parentAdapter.config.pushoverWaitToSend > 0) {
                    if (PushoverTimerId == null) {
                        PushoverMessage = msg;
                        PushoverTimerId = parentAdapter.setTimeout(sendNotificationPushover, parentAdapter.config.pushoverWaitToSend * 1000);
                    } else {
                        PushoverMessage += "\r\n" + msg;
                    }
                } else {
                ...
                if (parentAdapter.config.whatsappWaitToSend > 0) {
                    if (WhatsappTimerId == null) {
                        WhatsappMessage = msg;
                        WhatsappTimerId = parentAdapter.setTimeout(sendNotificationWhatsApp, parentAdapter.config.whatsappWaitToSend * 1000);
                    } else {
                        WhatsappMessage += "\r\n" + msg;
                    }
                } else {
                ...
                if (parentAdapter.config.signalWaitToSend > 0) {
                    if (SignalTimerId == null) {
                        SignalMessage = msg;
                        SignalTimerId = parentAdapter.setTimeout(sendNotificationSignal, parentAdapter.config.signalWaitToSend * 1000);
                    } else {
                        SignalMessage += "\r\n" + msg;
                    }
                } else {
                ...
                if (parentAdapter.config.discordWaitToSend > 0) {
                    if (DiscordTimerId == null) {
                        DiscordMessage = msg;
                        DiscordTimerId = parentAdapter.setTimeout(sendNotificationDiscord, parentAdapter.config.discordWaitToSend * 1000);
                    } else {
                        DiscordMessage += "\r\n" + msg;
                    }
                } else {
