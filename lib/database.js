...
async function CheckSensors4Room(room) {
    ...
    if (room.sensors != null) {
        ...
        for (let i = 0; i < room.sensors.length; i++) {
            ...
            const bRet = checkWindowSensor(room.sensors[i], room.Name);
            //ein Fenster reicht...
            if (bRet) {
                windowIsOpen = bRet;
            }
...
function checkWindowSensor(sensor, roomName) {
    ...
    switch (sensor.DataType) {
        ...
        default:
            ...
            parentAdapter.log.debug("Sensor state " + sensor.state + " ?= false" );
        ...
        case "boolean":
            ...
            parentAdapter.log.debug("Sensor state " + sensor.state + " ?= " + sensor.valueClosed);

            if (sensor.state != sensor.valueClosed) {
                windowIsOpen = true;
        ...
        case "string":
            ...
            parentAdapter.log.warn("Sensor state " + sensor.state + " ?= " + sensor.valueClosed);
