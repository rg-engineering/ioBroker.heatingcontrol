    ...
    try {
        ...
        if (theRoom != null) {
            ...
            if (state == "Sensor") {
                ...
                parentAdapter.log.debug("Change Status target temperature in " + theRoom.Name + " to " + val + " (profile " + theRoom.CurrentProfileTarget + ")");

                if (theRoom.State == "normal") {

                    await CheckStartManualMode(theRoom, val);
                    if (theRoom.State != "normal") {
                        changed = true;
                    }
                }
                    ...
                    if (theRoom.State == "manual") {
                        ...
                        await CheckStopManualMode(theRoom, val);
                        if (theRoom.State != "manual") {
                            changed = true;
                        }
...
async function CheckStartManualMode(room, target) {
    ...
    //standard fall: nicht reduziert und in normal-Modus
...
async function CheckOverride(room) {
    ...
    if (overrideTemp != null && overrideTime != null) {
        ...
        if (temperature > 0 && (parseInt(times[0]) > 0 || parseInt(times[1]) > 0)) {
            ...
            parentAdapter.log.debug(room.Name + " reset to normal after override");
            room.State = "normal";
