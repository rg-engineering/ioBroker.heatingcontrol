...
const SetValveProtectionCallback = require("./cronjobs").SetValveProtectionCallback;
...
//while a valve protection cycle runs, ignore the adapter's own thermostat writes (no feedback)
let ValveProtectionActive = false;
...
async function CreateDatabase(adapter, language) {
    ...
    //register valve protection cycle so the daily cron can trigger it
    SetValveProtectionCallback(OnValveProtectionTrigger);
        ...
        if (temperature > 0 && (parseInt(times[0]) > 0 || parseInt(times[1]) > 0)) {
            ...
            if (room.OverrideTimerId == null) {
                ...
                if (parseInt(parentAdapter.config.OverrideMode) != 1) {
                    ...
                    parentAdapter.log.debug(room.Name + " override mode until next profile point, no timer started ");
                }
...
async function CheckStateChangeDevice(id, state) {
    ...
    if (!bRet && !ValveProtectionActive && parentAdapter.config.UseChangesFromThermostat > 1) {
...
async function CheckSensors4Room(room) {
    ...
    parentAdapter.log.debug("CheckSensors4Room for " + room.Name + " " + JSON.stringify(room.sensors));

    if (room.sensors != null) {

        for (let i = 0; i < room.sensors.length; i++) {

            if (room.sensors[i].isActive) {
                parentAdapter.log.debug("CheckSensors4Room first check OID " + room.sensors[i].OID);
...
//*******************************************************************
//
//writes the timestamp of the next planned valve protection run to the info datapoint
async function UpdateNextValveProtection() {
    try {
        if (!parentAdapter.config.UseValveProtection) {
            return;
        }

        const interval = parseInt(parentAdapter.config.ValveProtectionInterval, 10) || 14;
        const timeStr = parentAdapter.config.ValveProtectionTime || "11:00";
        ...
        const now = Date.now();
        const lastRun = await parentAdapter.getStateAsync("LastValveProtection");

        //earliest moment the cycle becomes due again
        let base = now;
        if (lastRun != null && lastRun.val) {
            base = lastRun.val + interval * 24 * 60 * 60 * 1000;
        }
        if (base < now) {
            base = now;
        }

        //next daily occurrence of the configured time at or after "base"
        ...
        await parentAdapter.setStateAsync("NextValveProtection", { ack: true, val: candidate.getTime() });
    } catch (e) {
        parentAdapter.log.error("exception in UpdateNextValveProtection [" + e + "]");
    }
}

//*******************************************************************
//
//called by the daily valve protection cron; decides if a cycle is due and runs it
async function OnValveProtectionTrigger() {
    try {
        if (!parentAdapter.config.UseValveProtection) {
            return;
        }

        if (ValveProtectionActive) {
            parentAdapter.log.debug("valve protection skipped: a cycle is already running");
            return;
        }

        //only outside the heating period
        const heatingPeriodActive = await parentAdapter.getStateAsync("HeatingPeriodActive");
        if (heatingPeriodActive == null || heatingPeriodActive.val) {
            parentAdapter.log.debug("valve protection skipped: heating period is active");
            await UpdateNextValveProtection();
            return;
        }

        const now = Date.now();

        //if the next-run timestamp is set, use it; otherwise fall back to the interval logic
        const nextRun = await parentAdapter.getStateAsync("NextValveProtection");
        if (nextRun != null && nextRun.val) {
            if (now < nextRun.val) {
                parentAdapter.log.debug("valve protection not due yet (next run at " + new Date(nextRun.val).toISOString() + ")");
                return;
            }
        } else {
            //interval check (no next-run timestamp available)
            const interval = parseInt(parentAdapter.config.ValveProtectionInterval, 10) || 14;
            const lastRun = await parentAdapter.getStateAsync("LastValveProtection");
            if (lastRun != null && lastRun.val) {
                const daysSince = (now - lastRun.val) / (24 * 60 * 60 * 1000);
                if (daysSince < interval) {
                    parentAdapter.log.debug("valve protection not due yet (" + daysSince.toFixed(1) + "/" + interval + " days)");
                    await UpdateNextValveProtection();
                    return;
                }
            }
        ...
        await RunValveProtectionCycle();

        await parentAdapter.setStateAsync("LastValveProtection", { ack: true, val: now });

        await UpdateNextValveProtection();

    } catch (e) {
        parentAdapter.log.error("exception in OnValveProtectionTrigger [" + e + "]");
    }
}

//*******************************************************************
//
//moves every thermostat valve through a full stroke so it does not seize up:
//current value -> fully open (VisMaxProfilTemp) -> fully closed (ValveProtectionCloseTemp) -> back to previous value
async function RunValveProtectionCycle() {

    const openTemp = parseFloat(parentAdapter.config.VisMaxProfilTemp);
    const closeTemp = parseFloat(parentAdapter.config.ValveProtectionCloseTemp);
    const waitMs = (parseInt(parentAdapter.config.ValveProtectionWaitTime, 10) || 60) * 1000;

    if (isNaN(openTemp) || isNaN(closeTemp)) {
        parentAdapter.log.error("valve protection: open/close temperature not valid (" + openTemp + "/" + closeTemp + ")");
        return;
    }

    //only thermostats with a valid target OID
    const valid = Thermostats.filter(t => t.OID_Target != null && t.OID_Target.length > 0);
    if (valid.length === 0) {
        parentAdapter.log.debug("valve protection: no thermostats with target OID");
        return;
    }

    const delay = ms => new Promise(resolve => setTimeout(resolve, ms));

    parentAdapter.log.info("valve protection started for " + valid.length + " thermostat(s): open " + openTemp + " -> close " + closeTemp + ", wait " + (waitMs / 1000) + "s per phase");

    //remember current targets to restore them afterwards
    const previous = [];
    for (let i = 0; i < valid.length; i++) {
        const current = await parentAdapter.getForeignStateAsync(valid[i].OID_Target);
        previous[i] = (current != null) ? current.val : openTemp;
    }

    ValveProtectionActive = true;
    try {
        //phase 1: fully open
        for (let i = 0; i < valid.length; i++) {
            await setActorState(valid[i], openTemp);
        }
        await delay(waitMs);

        //phase 2: fully closed
        for (let i = 0; i < valid.length; i++) {
            await setActorState(valid[i], closeTemp);
        }
        await delay(waitMs);

        //phase 3: restore previous target
        for (let i = 0; i < valid.length; i++) {
            await setActorState(valid[i], previous[i]);
            valid[i].lastTargetFromDevice = previous[i];
        }
    } finally {
        ValveProtectionActive = false;
    }

    parentAdapter.log.info("valve protection finished");
}
...
async function StartStatemachine() {
    ...
    try {
        ...
        //publish the timestamp of the next planned valve protection run
        await UpdateNextValveProtection();
    ...
    if (newState != null &&  newState !== undefined) {
        ...
        switch (sensor.DataType) {
            ...
            case "boolean":
                ...
                parentAdapter.log.debug("Sensor state " + newState + " ?= " + sensor.valueClosed + " " + typeof sensor.valueClosed + " = " + Boolean(sensor.valueClosed));

                if (newState != Boolean(sensor.valueClosed)) {
                    windowIsOpen = true;
            ...
            case "number":
                ...
                if (newState != Number(sensor.valueClosed)) {
                    windowIsOpen = true;
            ...
            case "string":
                ...
                if (newState != String(sensor.valueClosed)) {
                    windowIsOpen = true;
