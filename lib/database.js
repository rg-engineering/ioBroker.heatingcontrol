...
async function SetThermostatTarget(roomName, thermostat, target) {
    ...
    if (thermostat != null) {
        parentAdapter.log.debug("Thermostat set timeout for " + thermostat.name + " " + thermostat.OID_Target + " to " + target);
...
async function ActorOnTimeout(actor) {
    try {
        ...
        if (actor != null) {
            parentAdapter.log.debug("Actor on timeout for " + actor.name + " " + actor.OID);
...
async function ActorOffTimeout(actor) {
    try {
        //const actor = findObjectByKey(Actors, "OID", actorOID);
        if (actor != null) {
            parentAdapter.log.debug("Actor off timeout for " + actor.name + " " + actor.OID);
...
async function CalculateTempOffset(room, value) {
    ...
    let datestring = ("0" + LastUpdateExtSensor.getDate()).slice(-2) + "." + ("0" + (LastUpdateExtSensor.getMonth() + 1)).slice(-2) + "." +
    ...
    parentAdapter.log.debug("got " + value.val + " from ext_Sensor LastUpdate " + datestring);
    ...
    if (thermostats != null) {
        ...
        for (let d = 0; d < thermostats.length; d++) {
            ...
            if (current != undefined && current != null) {
                ...
                datestring = ("0" + LastUpdateSensor.getDate()).slice(-2) + "." + ("0" + (LastUpdateSensor.getMonth() + 1)).slice(-2) + "." +
                ...
                parentAdapter.log.debug("got " + current.val + " from " + thermostats[d].OID_Current + " LastUpdate " + datestring );

                //timediff in min
                const timediff = (LastUpdateExtSensor - LastUpdateSensor) / 1000 / 60;
                ...
                if (MaxTimeDiff == 0 || Math.abs(timediff) < MaxTimeDiff) {
                    ...
                    parentAdapter.log.debug("!!! don't use value, timediff too long  " + timediff);
                }
            }
            else {
        ...
        if (validTemps > 0) {
            ...
            parentAdapter.log.warn("Temp offset calculation skipped, time difference too long");
        }
