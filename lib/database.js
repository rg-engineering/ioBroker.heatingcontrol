...
async function ChangeStatus(state, room, value) {
    ...
    if (ChangeStatusCmdList.length > 0) {
        ...
        parentAdapter.log.warn("ChangeStatus recall cmd from list " + cmd.room + " " + cmd.state + " " + JSON.stringify(cmd.value) + " (" + ChangeStatusCmdList.length + ")");
...
async function ChangeStatus_Part2(state, room, value) {
    ...
    try {
        ...
        if (theRoom != null) {
            ...
            else if (state == "CopyProfile") {
                parentAdapter.log.info("CopyProfile in " + theRoom.Name + " org profile " + value);
                await CopyProfile(theRoom.Name, value);

                //vis update fehlt
            }
            else if (state == "CopyPeriods") {
                parentAdapter.log.info("CopyPeriods in " + theRoom.Name + " org period " + value);
                await CopyPeriods(theRoom.Name, value);
...
async function UpdateVisState() {
    if (parentAdapter.config.UseVisFromPittini) {
        ...
        RoomsHtmlTable += "<tr>";
        RoomsHtmlTable += "<th> Raum </th>";
        RoomsHtmlTable += "<th> Soll </th>";
        if (parentAdapter.config.UseActors) {
            RoomsHtmlTable += "<th> Ist </th>";
        }
        RoomsHtmlTable += "<th> Status </th>";
        RoomsHtmlTable += "</tr>";
        ...
        for (let i = 0; i < Rooms.length; i++) {
            ...
            if (Rooms[i].CurrentTarget > -99) {
                RoomsHtmlTable += "<td>" + Rooms[i].CurrentTarget + "°C </td>";
            }
            else {
        ...
        if (thermostats != null) {
            for (let d = 0; d < thermostats.length; d++) {
                ...
                if (current != null && current.val != target && target > -99) {
                    ...
                    await parentAdapter.setForeignStateAsync(thermostats[d].OID_Target, { ack: true, val: target }); //avoid endless loop with ack = true
                }
                else {
        ...
        if (thermostats != null) {
            for (let d = 0; d < thermostats.length; d++) {
                ...
                if (target != null) {
                    ...
                    parentAdapter.log.error("could not read target OID " + thermostats[0].OID_Target + " in " + Rooms[r].Name);
                }
                ...
                if (current != null) {
                    ...
                    if (parentAdapter.config.UseActors) {
                        parentAdapter.log.error("could not read current OID " + thermostats[0].OID_Current + " in " + Rooms[r].Name);
                    }
                }
            }
        }
...
async function CopyPeriods(room, orgProfilePart) {
