                ...
                if (thermostats[d].OID_Target.length > 0) {
                    ...
                    if (current != null) {
                        if (current.val != target && target > -99) {
                            ...
                            DisableSubscriptionTarget(thermostats[d].OID_Target, room.Name);
...
//*******************************************************************
//
function DisableSubscriptionTarget(thermostats_oid, room) {

    parentAdapter.log.debug("unsubscribe " + thermostats_oid);

    parentAdapter.unsubscribeForeignStates(thermostats_oid);
...
function EnableSubscriptionTarget(thermostats_oid) {
    ...
    if (parentAdapter.config.UseChangesFromThermostat > 1) {
        ...
        parentAdapter.subscribeForeignStates(thermostats_oid);
    }
...
async function CheckSensors4Room(room) {
    ...
    if (room.SensorStatus != null) {

        for (let i = 0; i < room.SensorStatus.length; i++) {

            const value = await parentAdapter.getForeignStateAsync(room.SensorStatus[i].OID);

            if (value != null) {
                room.SensorStatus[i].state = value.val;
            }
            else {
                parentAdapter.log.error("could not read " + room.SensorStatus[i].OID);

            }

            if (room.SensorStatus[i].state == true) {
                windowIsOpen = true;
                parentAdapter.log.debug("one window open in " + room.Name + " " + room.SensorStatus[i].OID);
            }
        }
    }

    parentAdapter.log.debug("result for CheckSensors4Room in " + room.Name + " is " + windowIsOpen);
    return windowIsOpen;
...
function AddSensorState(room, data) {
    ...
    */
    ...
    const sensor = findObjectByKey(room.SensorStatus, "OID", data.OID);
    ...
    if (room.SensorStatus != null) {
        for (let i = 0; i < room.SensorStatus.length; i++) {
            if (room.SensorStatus[i].state!=false) {
                windowIsOpen = true;
                parentAdapter.log.debug("one window open in " + room.Name + " " + room.SensorStatus[i].OID);
            }
        }
    ...
    if (parentAdapter.config.UseSensors) {
        ...
        for (let s = 0; s < Sensors.length; s++) {
            ...
            if (value != null && typeof value !== undefined) {
                ...
                parentAdapter.log.debug("got " + value.val + " from " + Sensors[s].OID);
                ...
                await ChangeStatus("Sensor", Sensors[s].room, data);
            }
            else {
                parentAdapter.log.error("could not read " + Sensors[s].OID);
            }
...
async function CalculateTempOffset(room, value) {
    ...
    parentAdapter.log.debug("calculate temp offset for " + room.Name + " old value is " + room.TempOffset + ", got value " + JSON.stringify(value));
    ...
    if (thermostats != null) {
        ...
        for (let d = 0; d < thermostats.length; d++) {
            ...
            if (current != undefined && current != null) {
                ...
                //value.val = 20.79
                //thermostate:
                //got 23.7 from hm-rpc.1.000A1BE98A126E.1.ACTUAL_TEMPERATURE  -> 
                //got 22.6 from hm-rpc.1.000A1BE98A1104.1.ACTUAL_TEMPERATURE
        ...
        //debug only:
        parentAdapter.log.warn("TempOffset " + room.Name);
        parentAdapter.log.warn("(avg) Ist-Temperatur am Thermostat " + CurrentTemp);
        parentAdapter.log.warn("Ist-Temperatur am zusätzl. Sensor " + value.val);
        parentAdapter.log.warn("tempDiff " + tempDiff);
        parentAdapter.log.warn("(avg) TempOffset " + room.TempOffset);
