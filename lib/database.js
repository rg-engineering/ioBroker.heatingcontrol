...
async function CalculateTempOffset(room, value) {
    ...
    parentAdapter.log.debug("calculate temp offset for " + room.Name + " old value is " + room.TempOffset);
        ...
        for (let d = 0; d < thermostats.length; d++) {
            ...
            if (current != undefined && current != null) {
                if (thermostats.length > 1) {
                    CurrentTemp = (CurrentTemp + current.val) / 2;
                }
                else {
                ...
                room.TempOffset = (room.TempOffset + tempDiff) / 2;
            }
            else {
