...
async function GetActorsId4Room(actors, room) {
    ...
    parentAdapter.log.warn("got actors for " + room + " " + JSON.stringify(actors) + " " + JSON.stringify(Actors));
    return actors;
}
...
async function GetThermostatId4Room(thermostats, room) {
    ...
    for (let d = 0; d < thermostats.length; d++) {
        ...
        if (thermostats[d].isActive) {
            ...
            thermostats[d].id = Thermostats.length;
        }
    }

    parentAdapter.log.warn("got thermostats for " + room + " " + JSON.stringify(thermostats) + " " + JSON.stringify(Thermostats));
    return thermostats;
}
        ...
        if (parentAdapter.config.UseActors && ActorsWithoutThermostat.length > 0) {
            ...
            if (parseInt(parentAdapter.config.UseActorsIfNoThermostat) > 1) {
                ...
                for (let d = 0; d < ActorsWithoutThermostat.length; d++) {
                    ...
                    if (Actors[ActorsWithoutThermostat[d].id].OID === undefined) {
                        parentAdapter.log.error("undefined ActorsWithoutThermostat " + JSON.stringify(ActorsWithoutThermostat[d]));
                        parentAdapter.log.warn("Actors " + JSON.stringify(Actors));
                        break;
                    }
