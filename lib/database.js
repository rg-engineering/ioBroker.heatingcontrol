...
async function ChangeStatus(state, room, value) {
    ...
    if (ChangeStatusCmdList.length > 0) {
        ...
        ChangeStatus(cmd.state, cmd.room, cmd.value);
        parentAdapter.log.info("ChangeStatus recall cmd from list " + cmd.stata + "(" + ChangeStatusCmdList.length + ")");
    }
}
    ...
    try {
        ...
        if (theRoom != null) {
            if (state == "Sensor") {
                ...
                parentAdapter.log.debug("Change Status target temperature in " + theRoom.Name + " to " + val + " (" + theRoom.CurrentProfileTarget + ")");

                if (theRoom.State == "normal") {
                    if (theRoom.currentTarget != val) {
                        await CheckTargetFromThermostat(theRoom, val);
                        changed = true;
                    }
                }
                else {
                    if (theRoom.State == "manual") {
                        if (theRoom.CurrentProfileTarget == val) {
                            parentAdapter.log.info("Change Status reset manual mode in " + theRoom.Name);
                ...
                if (typeof value.target !== undefined && !isNaN(value.target)) { //this comes from cron job
                    ...
                    if (room.State == "manual") {
                        room.State = "normal";
                        parentAdapter.log.debug("reset manual mode to normal");
                    }
...
async function CheckTargetFromThermostat(room, target) {
    ...
    if (parseInt(parentAdapter.config.UseChangesFromThermostat) === 2) {
        ...
        parentAdapter.log.warn(room.Name + " got target from thermostat as new profil point: " + target);
    }
        ...
        /*
        if (room.IsInReduced) {
            parentAdapter.log.debug(room.Name + " we are in reduced " + room.CurrentReduced);
            if (room.CurrentReducedMode == "rel") {
                room.TemperatureManualMode += room.CurrentReduced;
                parentAdapter.log.debug(room.Name + " we are in rel reduced, new tempManual " + room.TemperatureManualMode);
            }
        }
        */
    }
