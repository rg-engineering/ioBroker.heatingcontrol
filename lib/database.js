...
const ActorLog = require("./logging").ActorLog;
const ThermostatLog = require("./logging").ThermostatLog;
const WindowLog = require("./logging").WindowLog;
const StartExtendedLog = require("./logging").StartExtendedLog;

const StartActors = require("./actor_ext").StartActors;
const setActorState = require("./actor_ext").setActorState;
...
async function UpdateVisState() {
    if (parentAdapter.config.UseVisFromPittini) {
        ...
        let colNameCurrentInt = GetTranslation(parentAdapter, "current_int", SystemLanguage);
        let colNameCurrentExt = GetTranslation(parentAdapter, "current_ext", SystemLanguage);
        let colNameOffset = GetTranslation(parentAdapter, "offset", SystemLanguage);
        ...
        for (let i = 0; i < Rooms.length; i++) {
            ...
            let currentTemperature = Rooms[i].CurrentTemperature;
            let targetTemperature = Rooms[i].CurrentTarget;
            let offset = Rooms[i].TempOffset;
            ...
            if (parentAdapter.config.VisUseSimple) {
                ...
                if (targetTemperature > -99 && Rooms[i].StatusLog[0].status != "no heating") {
...
async function SetRoomTemperature(room, temperature) {
    ...
    try {
        ...
        if (thermostats != null) {
            ...
            let InterThermostatDelay = parseInt(parentAdapter.config.InterThermostatDelay);
                    ...
                    if (current != null) {
                        if (current.val != target && target > -99) {
                            ...
                            if (InterThermostatDelay>0) {
                                ...
                                ThermostatLog(room.Name, thermostats[d].OID_Target, target);

                                //await parentAdapter.setForeignStateAsync(thermostats[d].OID_Target, { ack: false, val: target });
                                setActorState(thermostats[d], target);
                            }
                        }
                        else {
...
async function SetThermostatTarget(roomName, thermostat, target) {
    ...
    if (thermostat != null) {
        ...
        setActorState(thermostat, target);
    }
}
...
async function HandleActors(room) {
    ...
    try {
        if (parentAdapter.config.UseActors) {
            ...
            parentAdapter.log.debug("HandleActors for " + room.Name + " " + room.State + ",  timers " + delayCounter );

            let InterActorDelay = parseInt(parentAdapter.config.InterActorDelay);
            ...
            if (actors != null && actors.length > 0 && thermostats != null && thermostats.length > 0) {
                ...
                if ( !room.isActive || !room.HeatingPeriod)  {
                            ...
                            if (currentState != null && typeof currentState != undefined) {
                                if (currentState.val !== target) {
                                    ...
                                    if (InterActorDelay > 0) {
                                        ...
                                        setActorState(actors[d], target);
                                        ActorLog(actors[d].OID, target);
                                    }
                    ...
                    if (current != null && target != null) {
                        ...
                        let value2set=await regulator_GetStatus(room, current.val, target.val);
                                    ...
                                    if (value2set == false) {
                                        if (parseInt(parentAdapter.config.ActorBeforeOffDelay) > 0) {
                                            ...
                                            if (InterActorDelay > 0) {
                                                ...
                                                setActorState(actors[d], false);
                                                ActorLog(actors[d].OID, false);
                                            }
                                        }
                                    }
                                        ...
                                        if (parseInt(parentAdapter.config.ActorBeforeOnDelay) > 0) {
                                            ...
                                            if (InterActorDelay > 0) {
                                                ...
                                                setActorState(actors[d], true);
                                                ActorLog(actors[d].OID, true);
                                            }
                                        }
                                    }
                                }
        ...
        for (let d = 0; d < devices.length; d++) {
            ...
            if (room != null) {
                ...
                if (newWindowState != devices[d].lastState) {
                    ...
                    parentAdapter.log.debug("nothing to do for " + devices[d].OID);
                }
            }
            bRet = true;
        }
...
async function CalculateTempOffset(room, value) {
    ...
    let LastUpdateExtSensor = new Date(value.ts);
    let datestringExt = ("0" + LastUpdateExtSensor.getDate()).slice(-2) + "." + ("0" + (LastUpdateExtSensor.getMonth() + 1)).slice(-2) + "." +
        ...
        for (let d = 0; d < thermostats.length; d++) {
            ...
            if (current != undefined && current != null) {
                ...
                if (MaxTimeDiff == 0 || Math.abs(timediff) < MaxTimeDiff) {
                    parentAdapter.log.debug("use value ");
                ...
                for (let d = 0; d < ActorsWithoutThermostat.length; d++) {
                    ...
                    if (currentState != null && typeof currentState != undefined) {
                        if (currentState.val !== target) {
                            ...
                            setActorState(Actors[ActorsWithoutThermostat[d].id], target);
                        }
                        else {
