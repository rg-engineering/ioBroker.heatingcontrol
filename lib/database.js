                ...
                if (typeof value.target !== undefined && !isNaN(value.target)) { //this comes from cron job
                    if (parseInt(parentAdapter.config.OverrideMode) === 2) {
                        ...
                        if (theRoom.IsInOverride) {
                            parentAdapter.log.debug(room.Name + " new profile point, but still in override: need to stop override first");
                            theRoom.IsInOverride = false;
                            theRoom.overrideTemp = 0;
                            theRoom.State = "auto";
                        }
...
async function UpdateVisState() {
    if (parentAdapter.config.UseVisFromPittini) {
        ...
        let colNameRoom = GetTranslation("room", SystemLanguage);
        let colNameTarget = GetTranslation("target", SystemLanguage);
        let colNameCurrent = GetTranslation("current", SystemLanguage);
        let colNameActuator = GetTranslation("actuator", SystemLanguage);
        let colNameWindow = GetTranslation("window", SystemLanguage);
        let colNameState = GetTranslation("status", SystemLanguage);
        ...
        for (let i = 0; i < Rooms.length; i++) {
            ...
            if (parentAdapter.config.UseActors) {
                ...
                let stateNameOn = GetTranslation("on", SystemLanguage);
                let stateNameOff = GetTranslation("off", SystemLanguage);
            ...
            if (parentAdapter.config.UseSensors) {
                ...
                let windowStateNameOpen = GetTranslation("opened", SystemLanguage);
                let windowStateNameClosed = GetTranslation("closed", SystemLanguage);
    ...
    if (parentAdapter.config.UseSensors) {
        ...
        for (let room = 0; room < Rooms.length; room++) {
            ...
            if (Rooms[room].isActive) {
                const windowIsOpen = await CheckSensors4Room(Rooms[room]);

                Rooms[room].WindowIsOpen = windowIsOpen;
...
async function CalculateTempOffset(room, value) {
    ...
    if (thermostats != null) {
        ...
        parentAdapter.log.info("TempOffset calc " + tempValue + " / " + ctr + " = " + room.TempOffset);
