...
async function CalculateRoomTemperature(room) {
    ...
    if (room.State == "normal") {
        ...
        if (room.WindowIsOpen) {
            Temp2Set = await GetReducedTemperature(room.Name, Temp2Set, "WindowOpenDecrease");
            room.ReducedState = "WindowOpen";
            room.IsInReduced = true;
        }
        else if (!room.Present) {
            Temp2Set = await GetReducedTemperature(room.Name, Temp2Set, "AbsentDecrease");
            room.ReducedState = "Absent";
            room.IsInReduced = true;
        }
        else if (room.VacationAbsent) {
            Temp2Set = await GetReducedTemperature(room.Name, Temp2Set, "VacationAbsentDecrease");
            ...
            Temp2Set = await GetReducedTemperature(room.Name, Temp2Set, "PartyDecrease");
...
async function UpdateDPs(room) {
    ...
    await parentAdapter.setStateAsync(id + ".State", { val: room.State, ack: true });
