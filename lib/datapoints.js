    ...
    try {
        ...
        for (let room = 0; room < parentAdapter.config.rooms.length; room++) {
            if (parentAdapter.config.rooms[room].isActive) {
                ...
                await CreateDatapoint(key + ".CurrentTimePeriodTemperature", "state", "level.temperature", "number", "°C", true, false);
            ...
            for (let room = 0; room < parentAdapter.config.rooms.length; room++) {
                if (parentAdapter.config.rooms[room].isActive) {
                    ...
                    if (parseInt(parentAdapter.config.ProfileType, 10) === 1) {
                        ...
                        await FindUnusedDPs(id1 + ".Mo-Su.Periods.");
                    }
                        ...
                        await FindUnusedDPs(id1 + ".Mo-Fr.Periods.");
                        ...
                        await FindUnusedDPs(id1 + ".Sa-Su.Periods.");
                    }
                        ...
                        await FindUnusedDPs(id1 + ".Mon.Periods.");
                        ...
                        await FindUnusedDPs(id1 + ".Tue.Periods.");
                        ...
                        await FindUnusedDPs(id1 + ".Wed.Periods.");
                        ...
                        await FindUnusedDPs(id1 + ".Thu.Periods.");
                        ...
                        await FindUnusedDPs(id1 + ".Fri.Periods.");
                        ...
                        await FindUnusedDPs(id1 + ".Sat.Periods.");
                        ...
                        await FindUnusedDPs(id1 + ".Sun.Periods.");
                    }
...
async function FindUnusedDPs(baseId) {

    const start = Number(parentAdapter.config.NumberOfPeriods) + 1;
    for (let period = start ; period <= 10; period++) {

        const id = baseId + period;

        const obj = await parentAdapter.getObjectAsync(id);

        if (obj != null) {
            parentAdapter.log.warn("you could delete " + id);
        }
    }
}
