...
async function CreateDatapoints(adapter, language) {
    ...
    try {
        ...
        if (parentAdapter.config.UseValveProtection) {
            await CreateDatapoint("LastValveProtection", "state", "value", "number", "", true, true);
            await CreateDatapoint("NextValveProtection", "state", "date", "number", "", true, false);
        }
        ...
        if (parentAdapter.config.HeatingPeriodAsDP && parentAdapter.config.UseFixHeatingPeriod) {
            await CreateDatapoint("FixHeatingPeriodStart", "state", "value", "string", "", true, true);
            await CreateDatapoint("FixHeatingPeriodEnd", "state", "value", "string", "", true, true);
        }
        ...
        if (parentAdapter.config.UseVisFromPittini) {
            ...
            await CreateDatapoint("vis.RoomValues.TemperatureIfNoHeatingPeriod", "state", "level.temperature", "number", "°C", true, true);
            ...
            await CreateDatapoint("vis.translations.cardHzngProfilParam.TemperatureIfNoHeatingPeriod", "state", "level.temperature", "string", "", true, false);
...
async function SetDefaults() {
    ...
    try {
        ...
        if (parentAdapter.config.HeatingPeriodAsDP && parentAdapter.config.UseFixHeatingPeriod) {
            await SetDefault("FixHeatingPeriodStart", "01.10");
            await SetDefault("FixHeatingPeriodEnd", "01.04");
        }
        ...
        if (parentAdapter.config.UseVisFromPittini) {
            ...
            await SetDefault("vis.RoomValues.TemperatureIfNoHeatingPeriod", 20);
...
async function SetDefaultVisTranslatedText() {
    ...
    await parentAdapter.setStateAsync("vis.translations.cardHzngProfilParam.TemperatureIfNoHeatingPeriod", { ack: true, val: GetTranslation(parentAdapter,"TemperatureIfNoHeatingPeriod", SystemLanguage) });
...
async function SubscribeAllStates() {
    ...
    if (parentAdapter.config.HeatingPeriodAsDP && parentAdapter.config.UseFixHeatingPeriod) {
        parentAdapter.subscribeStates("FixHeatingPeriodStart");
        parentAdapter.subscribeStates("FixHeatingPeriodEnd");
    }
    ...
    //pittini vis
    if (parentAdapter.config.UseVisFromPittini) {
        parentAdapter.log.debug("subscribe Vis-DPs ");
