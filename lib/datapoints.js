    ...
    try {
        ...
        for (let room = 0; room < parentAdapter.config.rooms.length; room++) {
            if (parentAdapter.config.rooms[room].isActive) {
                ...
                await CreateDatapoint(key + ".CurrentTimePeriodTemperature", "state", "level.temperature", "number", "", true, false);
                ...
                await CreateDatapoint(key + ".TemperaturOverride", "state", "level.temperature", "number", "", true, true);
