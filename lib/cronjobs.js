...
function CreateCron4PowerInterruptions() {
    ...
    if (parentAdapter.config.PowerInterruptions != null && parentAdapter.config.PowerInterruptions.length > 0) {
        parentAdapter.log.info("set cron for Power Interruptions " + parentAdapter.config.PowerInterruptions.length);


        for (let i = 0; i < parentAdapter.config.PowerInterruptions.length; i++) {

            let StartPi = parentAdapter.config.PowerInterruptions[i].Start.split(/[:.,/ -]/);
            let EndPi = parentAdapter.config.PowerInterruptions[i].End.split(/[:.,/ -]/);
