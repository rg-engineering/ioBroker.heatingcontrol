...
let cbValveProtection = null;

let heatingPeriodStartId = -1;
let heatingPeriodEndId = -1;

//register the callback that runs the valve protection cycle (implemented in database.js)
function SetValveProtectionCallback(callback) {
    cbValveProtection = callback;
}
...
async function CreateCronJobs(adapter, currentProfile, callback, rooms) {
    try {
        ...
        //cron for valve protection
        CreateCron4ValveProtection();
...
function CronStop() {
    ...
    heatingPeriodStartId = -1;
    heatingPeriodEndId = -1;
}
/*
...
*/
async function CreateCron4HeatingPeriod() {
    ...
    if (parentAdapter.config.UseFixHeatingPeriod) {
        ...
        let HeatingPeriodStartRaw = parentAdapter.config.FixHeatingPeriodStart;
        let HeatingPeriodEndRaw = parentAdapter.config.FixHeatingPeriodEnd;

        if (parentAdapter.config.HeatingPeriodAsDP) {
            const start = await parentAdapter.getStateAsync("FixHeatingPeriodStart");
            const end = await parentAdapter.getStateAsync("FixHeatingPeriodEnd");

            if (start && start.val) {
                HeatingPeriodStartRaw = start.val;
            }
            if (end && end.val) {
                HeatingPeriodEndRaw = end.val;
            }
        }

        // validate function: expects a string like "24.12", "01.10", "24/12", "24-12" optionally with trailing dot
        function parseDayMonth(raw, name) {
            if (typeof raw !== "string") {
                parentAdapter.log.error(name + " is not a string: " + JSON.stringify(raw));
                return null;
            }
            const m = raw.toString().trim().match(/^\s*(\d{1,2})\s*[.,/\-\s]\s*(\d{1,2})\.?\s*$/);
            if (!m) {
                parentAdapter.log.error(name + " not valid (expected day.month without year) : " + raw);
                return null;
            }
            const day = parseInt(m[1], 10);
            const month = parseInt(m[2], 10);
            if (isNaN(day) || isNaN(month) || day < 1 || day > 31 || month < 1 || month > 12) {
                parentAdapter.log.error(name + " contains invalid day or month: " + raw);
                return null;
            }
            // return as strings to keep compatibility with existing code using indexes
            return [("0" + day).slice(-2), ("0" + month).slice(-2)];
        }

        const HeatingPeriodStart = parseDayMonth(HeatingPeriodStartRaw, "FixHeatingPeriodStart");
        const HeatingPeriodEnd = parseDayMonth(HeatingPeriodEndRaw, "FixHeatingPeriodEnd");

        if (HeatingPeriodStart && HeatingPeriodEnd) {
            try {
                // If previous heating period jobs exist, remove them first to avoid duplicates
                try {
                    if (heatingPeriodStartId >= 0 && cronJobs[heatingPeriodStartId]) {
                        parentAdapter.log.debug("remove existing HeatingPeriodStart cron job #" + heatingPeriodStartId);
                        try {
                            cronJobs[heatingPeriodStartId].stop();
                        } catch (e2) {
                            parentAdapter.log.debug("error stopping old HeatingPeriodStart cron job: " + e2);
                        }
                        cronJobs[heatingPeriodStartId] = null;
                        heatingPeriodStartId = -1;
                    }
                    if (heatingPeriodEndId >= 0 && cronJobs[heatingPeriodEndId]) {
                        parentAdapter.log.debug("remove existing HeatingPeriodEnd cron job #" + heatingPeriodEndId);
                        try {
                            cronJobs[heatingPeriodEndId].stop();
                        } catch (e2) {
                            parentAdapter.log.debug("error stopping old HeatingPeriodEnd cron job: " + e2);
                        }
                        cronJobs[heatingPeriodEndId] = null;
                        heatingPeriodEndId = -1;
                    }
                } catch (eRem) {
                    parentAdapter.log.debug("exception while removing old heating period cron jobs [" + eRem + "]");
                }
                ...
                heatingPeriodEndId = nextCron;

            } catch (e) {
                parentAdapter.log.error("exception in CreateCron4HeatingPeriod [" + e + "]");
            }
        } else {
            parentAdapter.log.error("heating period not valid " + HeatingPeriodStartRaw + " / " + HeatingPeriodEndRaw);
        }
    }
}
...
//*******************************************************************
//
function CreateCron4ValveProtection() {
    ...
    if (isNaN(hh) || isNaN(mm) || hh < 0 || hh > 23 || mm < 0 || mm > 59) {
        parentAdapter.log.error("valve protection time not valid: " + timeStr);
        return;
    }

    try {
        //daily at hh:mm; the interval and heating-period checks are done in the callback
        const cronString = mm + " " + hh + " * * *";

        parentAdapter.log.info("set cron for valve protection at " + timeStr);

        const nextCron = cronJobs.length;

        cronJobs[nextCron] = new CronJob(cronString,
            () => {
                if (cbValveProtection != null) {
                    cbValveProtection();
                }
            },
            () => parentAdapter.log.debug("cron job ValveProtection stopped"),
            true,
            timezone
        );

    } catch (e) {
        parentAdapter.log.error("exception in CreateCron4ValveProtection [" + e + "]");
    }
}
