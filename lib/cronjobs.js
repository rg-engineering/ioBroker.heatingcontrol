...
async function CreateTimerList(PeriodName, currentProfile) {
    ...
    for (let room = 0; room < parentAdapter.config.rooms.length; room++) {
        if (parentAdapter.config.rooms[room].isActive) {
            ...
            if (PeriodName == "Mo-Fr" && room.PublicHolidyToday && parentAdapter.config.PublicHolidayLikeSunday === true) {
                PeriodName = "Sa-Su";
                parentAdapter.log.warn("week end profile used for public holiday");
            }
            else if (PeriodName == "Mo-Fr" && room.HolidayPresent && parentAdapter.config.HolidayPresentLikeSunday === true) {
                PeriodName = "Sa-Su";
